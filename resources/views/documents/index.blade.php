@extends('layouts.default')

@section('content')
    <div class="main-content__top">
        <h1 class="main-content__title">
            {{ $title }}
        </h1>
        <a href="{{ route('documents.create') }}" class="link create-link">
            <svg width="30px" height="30px" viewBox="0 0 32 32" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:sketch="http://www.bohemiancoding.com/sketch/ns">
                <title>plus-circle</title>
                <desc>Created with Sketch Beta.</desc>
                <defs>   
            </defs>
                <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd" sketch:type="MSPage">
                    <g id="Icon-Set" sketch:type="MSLayerGroup" transform="translate(-464.000000, -1087.000000)" fill="#000000">
                        <path d="M480,1117 C472.268,1117 466,1110.73 466,1103 C466,1095.27 472.268,1089 480,1089 C487.732,1089 494,1095.27 494,1103 C494,1110.73 487.732,1117 480,1117 L480,1117 Z M480,1087 C471.163,1087 464,1094.16 464,1103 C464,1111.84 471.163,1119 480,1119 C488.837,1119 496,1111.84 496,1103 C496,1094.16 488.837,1087 480,1087 L480,1087 Z M486,1102 L481,1102 L481,1097 C481,1096.45 480.553,1096 480,1096 C479.447,1096 479,1096.45 479,1097 L479,1102 L474,1102 C473.447,1102 473,1102.45 473,1103 C473,1103.55 473.447,1104 474,1104 L479,1104 L479,1109 C479,1109.55 479.447,1110 480,1110 C480.553,1110 481,1109.55 481,1109 L481,1104 L486,1104 C486.553,1104 487,1103.55 487,1103 C487,1102.45 486.553,1102 486,1102 L486,1102 Z" id="plus-circle" sketch:type="MSShapeGroup">
            </path>
                    </g>
                </g>
            </svg>
            Créer un document
        </a>
    </div>

    <table class="table-auto">
        <thead>
            <tr>
                <th class="px-4 py-2">Client</th>
                <th class="px-4 py-2">Type du document</th>
                <th class="px-4 py-2">Reference</th>
                <th class="px-4 py-2">Total hors TVA</th>
                <th class="px-4 py-2">TVA</th>
                <th class="px-4 py-2">Total toutes charges comprises</th>
                <th class="px-4 py-2">Statut</th>
                <th class="px-4 py-2">Commentaire</th>
                <th class="px-4 py-2">Produits</th>
                <th class="px-4 py-2">Actions</th>
            </tr>
        </thead>
        <tbody>
            @foreach ($documents as $document)
                <tr>
                    @if($document->customer)
                        <td class="border px-4 py-2">{{ $document->customer->name }}</td>
                    @else 
                        <td class="border px-4 py-2">Pas de client</td>
                    @endif
                    @if($document->documenttype)
                        <td class="border px-4 py-2">{{ $document->documenttype->name }}</td>
                    @else 
                        <td class="border px-4 py-2">Pas de type de document</td>
                    @endif
                    <td class="border px-4 py-2">{{ $document->reference }}</td>
                    <td class="border px-4 py-2">{{ $document->totalhvat }} €</td>
                    <td class="border px-4 py-2">{{ $document->vat }} %</td>
                    <td class="border px-4 py-2">{{ $document->totalttc }} €</td>
                    <td class="border px-4 py-2">{{ $document->statut }}</td>
                    <td class="border px-4 py-2">{{ $document->comment }}</td>
                    @if($document->products)
                    <td class="border px-4 py-2">
                        <ul>
                            @foreach ($document->products as $product)
                                <li>{{ $product->name }}</li>
                            @endforeach
                        </ul>
                    </td>
                    @endif
                    <td class="table-actions">
                        <a href="{{ route('documents.show', $document->id) }}"
                            class="link see-link">Voir</a>
                        <a href="{{ route('documents.edit', $document->id) }}"
                            class="link update-link">Modifier</a>
                        <form class="inline-block" action="{{ route('documents.destroy', $document->id) }}" method="POST"
                            onsubmit="return confirm('Are you sure?');">
                            <input type="hidden" name="_method" value="DELETE">
                            <input type="hidden" name="_token" value="{{ csrf_token() }}">
                            <button type="submit" class="link delete-link">Supprimer</button>
                        </form>
                    </td>
                </tr>
            @endforeach
        </tbody>
    </table>

@stop